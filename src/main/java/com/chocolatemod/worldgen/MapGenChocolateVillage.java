...
public class MapGenChocolateVillage extends MapGenChocolateStructure
{
    /** A list of all the biomes villages can spawn in. */
    public static List villageSpawnBiomes = Arrays.asList(new BiomeGenBase[] {BiomeGenChocolateBase.chocolatePlains, BiomeGenChocolateBase.chocolateDesert, BiomeGenChocolateBase.chocolateForest});
    ...
    public MapGenChocolateVillage(Map p_i2093_1_)
    {
        this();
    ...
    {
        return new MapGenChocolateVillage.Start(this.worldObj, this.rand, p_75049_1_, p_75049_2_, this.terrainType);
    }
    ...
    public static class Start extends ChocolateStructureStart
        {
            ...
            public Start(World world, Random rand, int ChunkX, int ChunkZ, int ChunkY)
            {
                super(ChunkX, ChunkZ);
                List list = StructureChocolateVillagePieces.getStructureVillageWeightedPieceList(rand, ChunkY);
                StructureChocolateVillagePieces.Start start = new StructureChocolateVillagePieces.Start(world.getWorldChunkManager(), 0, rand, (ChunkX << 4) + 2, (ChunkZ << 4) + 2, list, ChunkY);
                ...
                {
                    ChocolateStructureComponent structurecomponent;

                    if (list1.isEmpty())
                    {
                        l = rand.nextInt(list2.size());
                        structurecomponent = (ChocolateStructureComponent)list2.remove(l);
                        structurecomponent.buildComponent(start, this.components, rand);
                        
                        System.out.println("My Village structure generated at " + ChunkX +"/" + ChunkY + "/" + ChunkZ + " coordinates");

                    }
                    else
                    {
                        l = rand.nextInt(list1.size());
                        structurecomponent = (ChocolateStructureComponent)list1.remove(l);
                        structurecomponent.buildComponent(start, this.components, rand);
                ...
                while (iterator.hasNext())
                {
                    ChocolateStructureComponent structurecomponent1 = (ChocolateStructureComponent)iterator.next();

                    if (!(structurecomponent1 instanceof StructureChocolateVillagePieces.Road))
                    {
                        ++l;
                    }
                }
