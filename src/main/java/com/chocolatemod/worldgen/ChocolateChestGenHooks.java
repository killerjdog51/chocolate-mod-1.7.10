...
import com.chocolatemod.worldgen.StructureChocolateVillagePieces.House2;
...
public class ChocolateChestGenHooks
{
  ...
  //  public static final String DUNGEON_CHEST            = "dungeonChest";
    ...
    private static final HashMap<String, ChocolateChestGenHooks> chestInfo = new HashMap<String, ChocolateChestGenHooks>();
    ...
    private static void init()
    {
        ...
        addInfo(MINESHAFT_CORRIDOR,       StructureChocolateMineshaftPieces.mineshaftChestContents,   3,  7);
   ...
   //     getInfo(PYRAMID_DESERT_CHEST).addItem(tmp);
    ...
    static void addDungeonLoot(ChocolateChestGenHooks dungeon, ItemStack item, int weight, int min, int max)
    {
    ...
    {
        chestInfo.put(category, new ChocolateChestGenHooks(category, items, min, max));
    }
     ...
     */
    public static ChocolateChestGenHooks getInfo(String category)
    {
        if (!chestInfo.containsKey(category))
        {
            chestInfo.put(category, new ChocolateChestGenHooks(category));
        }
    ...
    public ChocolateChestGenHooks(String category)
    {
        this.category = category;
    }

    public ChocolateChestGenHooks(String category, WeightedRandomChestContent[] items, int min, int max)
    {
        this(category);
    ...
    public WeightedRandomChestContent[] getItems(Random rnd)
    {
        ...
        for (WeightedRandomChestContent orig : contents)
        {
            ...
            if (item != null)
            {
                WeightedRandomChestContent n = ItemRegistry.getChestGenBase(this, rnd, orig);
                if (n != null)
                {
                    ret.add(n);
                }
    ...
    public ItemStack getOneItem(Random rand)
    {
        ...
        ItemStack[] stacks = ChocolateChestGenHooks.generateStacks(rand, item.theItemId, item.theMinimumChanceToGenerateItem, item.theMaximumChanceToGenerateItem);
